...
export const StatsRootView = () => {
  return (
    <VerticalBorderLayout
      top={<TitleBar text="Stats" left={<TitleBar.SettingsButton />} />}
    >
...
export const DayStats = ({ isoDate }) => {
  ...
  const {
    schema: {
      spaces: {
        ...
        PhysicalActivity: { values: physicalActivities }
      }
    }
  } = useSchema();
  ...
  const typedEntries = entries
    ...
    .filter(
      (typedEntry) => typedEntry.type.entryTypeClass === "PHYSICAL_ACTIVITY"
    );

  const duration = typedEntries.reduce(
    (accu, typedEntry) => accu + typedEntry.entry.duration,
    0
  );
  const energy = typedEntries.reduce(
    (accu, typedEntry) =>
      accu +
      (typedEntry.entry.duration / 3600) *
        physicalActivities.find(
          (physicalActivity) =>
            physicalActivity.id === typedEntry.type.physicalActivity
        ).energy,
    0
  );

  return (
    <CalendarSection title={toRelativeDate(isoDate)}>
      <ListItem>
        <ListItem.SubTitle>
          Physical training duration: <DurationOutput value={duration || 0} />
        </ListItem.SubTitle>
      </ListItem>
      <ListItem>
        <ListItem.SubTitle>
          Energy spent: {Math.round(energy)}
        </ListItem.SubTitle>
      </ListItem>
    </CalendarSection>
  );
